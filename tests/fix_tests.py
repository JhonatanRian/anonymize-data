...
def patch_file(filepath):
    ...
    if "test_dict_strategy.py" in filepath:
        ...
        content = content.replace("DefaultDictAnonymizationStrategy()", "DefaultDictAnonymizationStrategy(dispatch_value_mask)")
        content = content.replace("KeyBasedDictAnonymizationStrategy(selected_keys=[\"key1\"])", "KeyBasedDictAnonymizationStrategy([\"key1\"], dispatch_value_mask)")
        content = content.replace("KeyAsTypeMaskDictAnonymizationStrategy()", "KeyAsTypeMaskDictAnonymizationStrategy(dispatch_value_mask)")
    ...
    def repl_assert_equal(m):
        ...
        if expected.startswith('"') or expected.startswith("'"):
            ...
            if raw_str in ["invalid-cep", "123", "invalid-cnpj", "invalid-email", "abc", "12", "invalid-pis", "invalid-rg", "invalid-cpf"]:
    ...
    content = re.sub(r'self\.assertEqual\((anonymize_\w+\(.*?\)),\s*(.*?)\)', repl_assert_equal, content)

    # For test_invalid_cpf which has: result = MaskStr(cpf, "cpf").anonymize(); self.assertEqual(result, cpf)
    # where cpf = "12345678910"
    content = content.replace('self.assertEqual(result, cpf)', 'self.assertEqual(result, "*" * len(str(cpf)))')

    # test_mask_string.py
    if "test_mask_string.py" in filepath:
        content = content.replace('self.assertEqual(mask_string.anonymize(), cpf_invalid)', 'self.assertEqual(mask_string.anonymize(), "*" * len(cpf_invalid))')
